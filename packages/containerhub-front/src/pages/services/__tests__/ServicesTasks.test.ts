...
test('services retain the original expandable task list and separate logs page', async () => {
    const [servicesPage, router, logsPage, rest, tasksPanel] = await Promise.all([
        ...
        readFile(new URL('../ServiceTasksPanel.vue', import.meta.url), 'utf8')
    ])
    ...
    assert.match(servicesPage, /<ServiceTasksPanel/)
    ...
    assert.match(tasksPanel, /emit\('terminal', task, 'sh'\)/)
    assert.match(servicesPage, /@terminal="\(task, shell\) => openTerminal\(task, shell\)"/)
