...
### Corte posterior: decodificación de logs

Se trasladó literalmente el decodificador puro de `ServiceService.ts` a `packages/containerhub-back/src/modules/services/services/TaskLogDecoder.ts`; la fachada conserva los exports y el I/O, permisos y filtros de streaming siguen en su sitio. El harness de `ServiceLogs.test.ts` necesitó el mismo compilador de validación que las pruebas vecinas para arrancar (fallaba antes del corte). GREEN previo 9/9 tras reparar el harness, RED de import del módulo nuevo, GREEN final 11/11 pruebas focales y `tsc --noEmit`. No se probaron Docker, WebSocket ni la API desplegada. El inventario [`second-pass.tsv`](second-pass.tsv) actualizado tiene **235 filas: 206 unchanged, 21 blocked, 8 refactored**. El resto de `ServiceService.ts` permanece bloqueado para extracciones adicionales; no se alteró `updateNetwork`.
