...
| G-03 | **Implementado; UI local verificada** | El tab del navegador actualiza título y favicon según la sección activa y restablece los valores de ContainerHub al salir de una sección especial. | El router original actualiza título/favicon (`router/index.js:15-30`) y las vistas de terminal, cluster/inspect, estadísticas y logs usan los assets especializados. | `router.afterEach` consume `meta.title`/`meta.favicon` (`packages/containerhub-front/src/router/index.ts:63-108,137-148`); Vite sirve los assets desde `packages/containerhub-front/public/`; `e2e/browser-tab.spec.ts:27-55` verifica cambios y reset en Chromium. | Mantener la prueba de navegador; no acoplar el título a cargas API salvo que se solicite recuperar los nombres contextuales de servicio/contenedor. |
